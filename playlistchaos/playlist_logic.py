...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # Fix: chill keywords describe genres ("lofi", "ambient"), so match them
    # against the genre, not the title. Matching the title meant a song like
    # "Sleep Sound" was only chill-flagged by coincidence of its title text.
...
def merge_playlists(a: PlaylistMap, b: PlaylistMap) -> PlaylistMap:
    ...
    for key in set(list(a.keys()) + list(b.keys())):
        # Fix: start from a *copy* of a's list. Previously merged[key] aliased
        # the list owned by `a`, so the following extend() mutated the caller's
        # input in place (and repeated merges kept growing it).
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # Fix: the ratio should be hype songs out of *all* songs. Previously
    # total = len(hype), making hype_ratio always 1.0 (or 0.0 when empty).
    total = len(all_songs)
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # Fix: average energy over every song, not just the hype ones. The
        # numerator previously summed only hype energy while the denominator
        # counted all songs, understating the true average.
...
def search_songs(
    ...
    for song in songs:
        ...
        # Fix: check whether the query is a substring of the field value, not
        # the other way around. The original `value in q` only matched when the
        # whole field happened to be contained in the (usually shorter) query.
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    # Fix: random.choice([]) raises IndexError. Guard the empty case so we
    # honor the documented contract of returning None instead of crashing.
    if not songs:
        return None
